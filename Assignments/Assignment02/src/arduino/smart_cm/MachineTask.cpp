...
// Loop executed every MachineTask period (100ms)
void MachineTask::tick() {
    switch (state) {
      
        // Initial state: showing welcome
        case Boot:
            if (periodCounter == 0) {
                lcd.clear();
                lcd.setCursor(0,0);
        ...
        // Ready state: waiting for user input
        case Ready:
        ...
        // Selecting state: the user is selecting the product
        case Selecting:
            if (testPeriodCounter >= T_CHECK / MACHINE_TASK_PERIOD) {
                selfTests++;
                state = Testing;
        ...
        // Making state: making the product
        case Making:
        ...
        // Assistance state: all the products are unavailable or a self test failed
        // Waiting for user input from Java app
        case Assistance:
            if (periodCounter == 0) {
                lcd.clear();
        ...
        // WaitRemove state: waiting for the user to take the finished product
        case WaitRemove:
            if (periodCounter == 0) {
        ...
        // Idle state: the machine is in deep sleep mode.
        // Waiting to be awakened from the presence sensor.
        case Idle:
            if (periodCounter <= 10) {
                modality = "idle";
                lcd.clear();
        ...
        // Testing state: machine doing a self test
        case Testing:
...
// Function called when machine is awakened
